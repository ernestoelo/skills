...
If you need to fill out a PDF form, first check to see if the PDF has fillable form fields. Run this script from this file's directory:
 `python shared/scripts/check_fillable_fields.py <file.pdf>`, y según el resultado ve a la sección correspondiente.

# Fillable fields
If the PDF has fillable form fields:
- Ejecuta este script desde shared/scripts/: `python shared/scripts/extract_form_field_info.py <input.pdf> <field_info.json>`. Creará un JSON con los campos.
```
[
  {
...
```
- Convierte el PDF a PNGs (una imagen por página) con:
`python shared/scripts/convert_pdf_to_images.py <file.pdf> <output_directory>`
...
```
- Ejecuta el script para rellenar el PDF:
`python shared/scripts/fill_fillable_fields.py <input pdf> <field_values.json> <output pdf>`
...
Ejecuta este script para extraer etiquetas, líneas y checkboxes:
`python shared/scripts/extract_form_structure.py <input.pdf> form_structure.json`
...
Antes de rellenar, valida los bounding boxes:
`python shared/scripts/check_bounding_boxes.py fields.json`
...
`python shared/scripts/convert_pdf_to_images.py <input.pdf> <images_dir/>`
...
`python shared/scripts/fill_pdf_form_with_annotations.py <input.pdf> fields.json <output.pdf>`

## Step 4: Verify Output

Convert the filled PDF to images and verify text placement:
`python shared/scripts/convert_pdf_to_images.py <output.pdf> <verify_images/>`
