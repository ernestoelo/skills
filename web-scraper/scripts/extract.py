...
class MarkdownPurist(HTMLParser):
    def __init__(self):
        ...
        self.banned_class_substrings = [
            ...
            "surveypopup",
            "absolute",
            "group/link",  # Captura botones flotantes y enlaces fantasma
        ]
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag in self.banned_tags or any(
            ban in classes for ban in self.banned_class_substrings
        ):
            self.ignored_depth = 1
            return
        ...
        if not self.recording:
            is_start = False
            if tag == "article" or (
                "role" in attr_dict and "main" in attr_dict["role"]
            ):
                is_start = True
            else:
    ...
    def handle_data(self, data):
        if self.recording and self.ignored_depth == 0:
            ...
            if text.strip() in self.banned_text:
                return
            if text == "\u200b":
                return
