...
You can configure additional locations where VS Code searches for skills by using the chat.agentSkillsLocations
...
Example: Web application testing skill
    ...
    4.
     ...
     Use Playwright's locators to find elements (prefer role-based selectors)
...
Example: GitHub Actions debugging skill
    ...
    `summarize_job_log_failures`
     ...
     tool to get an AI summary of the logs for failed jobs

    3.

     If you need more information, use the 
    ...
    `get_workflow_run_logs`
     ...
     tool to get the full failure logs
    ...
    : Verify action versions and dependencies are compatible
    ...
    : Consider splitting long-running jobs or increasing timeout values
...
Always review shared skills before using them to ensure they meet your requirements and security standards. VS Code's [terminal tool](/docs/copilot/chat/chat-tools#_terminal-commands) provides controls for script execution, including [auto-approve options](/docs/copilot/chat/chat-tools#_automatically-approve-terminal-commands) with configurable allow-lists and tight controls over which code runs. Learn more about [security considerations](/docs/copilot/security#_automated-approval) for auto-approval features.
